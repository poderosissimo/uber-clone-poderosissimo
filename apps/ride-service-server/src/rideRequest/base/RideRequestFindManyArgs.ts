/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RideRequestWhereInput } from "./RideRequestWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { RideRequestOrderByInput } from "./RideRequestOrderByInput";

@ArgsType()
class RideRequestFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => RideRequestWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => RideRequestWhereInput, { nullable: true })
  @Type(() => RideRequestWhereInput)
  where?: RideRequestWhereInput;

  @ApiProperty({
    required: false,
    type: [RideRequestOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [RideRequestOrderByInput], { nullable: true })
  @Type(() => RideRequestOrderByInput)
  orderBy?: Array<RideRequestOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { RideRequestFindManyArgs as RideRequestFindManyArgs };
